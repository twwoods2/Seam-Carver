...
public:
   ...
   * 
   */
  int** Empty2dArray(int num_rows,int num_cols) const;
  int* FindMinValPath(int** energy_arr) const;
  int DeltaColorValue(Pixel dominant, Pixel latter) const;
  int* HorizontalHelper(int** arr, int index) const;
private:
  ImagePPM image_;
  int height_ = 0;
  int width_ = 0;
