...
Pixel ImagePPM::GetPixel(int row, int col) const { // not implemented 
  return pixels_[row][col];
}
int ImagePPM::GetMaxColorValue() const { //not implemented
  return max_color_value_;
}
void ImagePPM::SetPixel(Pixel** new_array, int width, int height) {
    Clear();
    pixels_ = new_array;
    height_ = height;
    width_ = width;
